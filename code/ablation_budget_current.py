...
OUTPUT_DIR = PROJECT_ROOT / 'results' / 'budget_ablation_current'
...
def main():
    ...
    for length_frac in LENGTH_EPS_FRACS:
        alias = f'retrasyn_pst_{PROFILE}_len{length_frac.replace(".", "p")}'
        compare_methods.append(alias)
        ...
        gen_log = OUTPUT_DIR / f'len{length_frac.replace(".", "p")}_generate.log'
        cmd = [
            str(PYTHON_EXE),
            ...
            '--w', WINDOW,
            '--phi', PHI,
            '--pst_length_eps_frac', length_frac,
            *EXTRA_ARGS,
        ]
    ...
    eval_log = OUTPUT_DIR / 'budget_current_evaluate.log'
    eval_cmd = [
        str(PYTHON_EXE),
    ...
    json_path = OUTPUT_DIR / f'{DATASET}_{PROFILE}_g{GRID_NUM}_budget_ablation_current.json'
    csv_path = OUTPUT_DIR / f'{DATASET}_{PROFILE}_g{GRID_NUM}_budget_ablation_current.csv'
    png_path = OUTPUT_DIR / f'{DATASET}_{PROFILE}_g{GRID_NUM}_budget_ablation_current.png'

    json_path.write_text(json.dumps(rows, indent=2), encoding='utf-8')
    fieldnames = ['method', 'length_eps'] + METRIC_ORDER
    with csv_path.open('w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction='ignore')
        writer.writeheader()
        writer.writerows(rows)

    variant_rows = [row for row in rows if row['method'] != 'retrasyn']
    baseline = next((row for row in rows if row['method'] == 'retrasyn'), None)
    x = [row.get('length_eps', float('nan')) for row in variant_rows]
    ...
    fig.suptitle(f'Current Budget Ablation ({DATASET}, {PROFILE}, grid={GRID_NUM})')
    fig.tight_layout()
